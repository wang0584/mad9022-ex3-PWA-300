const APP = {
  ...
  isOnline: 'onLine' in navigator && navigator.onLine, //TODO: check navigator.onLine
  init: () => {
    //when the page loads (runs on every page load)
    APP.registerSW();
    APP.addListeners();
    APP.updateNavCount();
    APP.changeDisplay();  //update the online/offline css class
  },
  registerSW: () => {
  ...
  changeStatus: (ev) => {
    ...
    navigator.serviceWorker.ready.then(registration=>{
      registration.active.postMessage({ ONLINE: APP.isOnline, NAME: 'Xuefeng' });
    });
  ...
  changeDisplay: ()=>{
    if (APP.isOnline) {
      ...
      document.querySelector('.isonline').textContent = 'NOT-ONLINE';
    }
  },
  gotMessage: (ev) => {
